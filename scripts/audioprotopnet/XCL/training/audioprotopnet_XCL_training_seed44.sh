#!/bin/bash
#SBATCH --job-name=audioprotopnet_XCL_seed44
#SBATCH --output=/mnt/data/rheinrich/DBD/audioprotopnet/job_logs/audioprotopnet_XCL_seed44_%j.log
#SBATCH --ntasks=1
...
echo "Starting experiment: audioprotopnet_convnext_XCL_seed44 at $(date)"
srun --exclusive python train_audioprotopnet.py experiment="XCL/audioprotopnet/training/audioprotopnet_convnext_1_prototype" seed=44 module.network.model.backbone_model.local_checkpoint="/mnt/oekofor/data/rheinrich/DBD/audioprotopnet/logs/train/runs/XCL/convnext/2024-08-13_202453/callback_checkpoints/convnext_XCL_04.ckpt"
if [[ $? -ne 0 ]]; then
    echo "Experiment audioprotopnet_convnext_XCL_seed44 failed at $(date)"
    exit 1
fi
