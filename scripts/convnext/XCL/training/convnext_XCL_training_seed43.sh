#!/bin/bash
#SBATCH --job-name=convnext_XCL_seed43
#SBATCH --output=/mnt/data/rheinrich/DBD/audioprotopnet/job_logs/convnext_XCL_seed43_%j.log
#SBATCH --ntasks=1
...
echo "Starting experiment: convnext_XCL_seed43 at $(date)"
srun --exclusive python train_benchmarks.py experiment="XCL/convnext/training/convnext" seed=43
if [[ $? -ne 0 ]]; then
    echo "Experiment convnext_XCL_seed43 failed at $(date)"
    exit 1
fi
